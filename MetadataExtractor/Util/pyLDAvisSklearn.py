"""
pyLDAvis sklearn (Own implementation to fix NMF inclusion)
===============
...
def _row_norm(dists):
    ...
    summedModel = dists.sum(axis=1)
    for i in range(len(summedModel)):
        if summedModel[i] == 0:
            dists[i] = 1.0 / len(dists[i])
        else:
            dists[i] = dists[i] / summedModel[i]
    return dists
...
def _extract_data(lda_model, dtm, vectorizer):
    ...
    assert term_freqs.shape[0] == len(
        vocab
    ), "Term frequencies and vocabulary are of different sizes, {} != {}.".format(
        term_freqs.shape[0], len(vocab)
    )

    assert (
        topic_term_dists.shape[1] == dtm.shape[1]
    ), "Topic-term distributions and document-term matrix have different number of columns, {} != {}.".format(
